...
public class UserComplPost implements MouseListener{
	JPanel contentPane;
	...
	int no=0;//글번호
	int pageCnt=0;
	public UserComplPost(int no,String title,String id,int like,int viewCnt,String date)   {
		...
		updateSaveBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ModifyPostPane modiPane=new ModifyPostPane(1,no,complVO.getComplTitle(),cbVO.getComplContent(),false);
		...
		if(!UserMain.mvo.getMemId().equals(id)) {
			...
			if(cbd.isThereComplBoard(UserMain.mvo.getMemId(),no)==0) {
					...
					public void mouseClicked(MouseEvent e) {
						...
						if (cbDAO.updateLikeCnt(no)) {
							...
							if (cbDAO.updateMemLike(id)) {
								...
								if (cbDAO.insertLikeBoardNo(UserMain.mvo.getMemId(), no)) {
		...
		wirteCmtBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(ccDAO.writeComplComt(no, lastnum+1,UserMain.mvo.getMemId(),content)) {
	...
	class CustomCmtPane extends JPanel {
		...
		public CustomCmtPane(int complNo,int comtNo,String memId,String content,int like,String date) {
			...
			{	
				...
				if(ccomtDao.isThereComplComt(UserMain.mvo.getMemId(), complNo, comtNo)==0) {
						...
						public void actionPerformed(ActionEvent e) {
							...
							if (ccomtDAO.updateLikeCnt(complNo,comtNo)) {
								...
								if (ccomtDAO.updateMemLike(memIdLabel.getText())) {
									...
									if (ccomtDAO.insertLikeBoardNo(UserMain.mvo.getMemId(), complNo, comtNo)) {
										JOptionPane.showMessageDialog(null, "좋아요누른게시물 추가성공");
									} else {
										JOptionPane.showMessageDialog(null, "작성자 좋아요 수 높이기 실패");
									}
			...
			deleteBtn.addMouseListener(new MouseListener() {
				...
				public void mouseClicked(MouseEvent e) {
					ComplComtDAO ccDAO = new ComplComtDAO();
					System.out.println(complNo+" "+comtNo);
