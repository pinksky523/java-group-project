...
import project.admin.memberInfo.MemberInfoManager;
import project.admin.qnaBoardMng.QnaPost;
import project.publicMain.PublicMain;
import project.user.complBoard.UserComplPost;
import project.user.main.UserMain;
import project.user.message.MessageBox;
import project.user.qnaBoard.UserQnaPost;

public class Mypage implements ActionListener, MouseListener {
	JPanel mypagePane, userInfoPane, pntPane, postAndRepl, complPageNoPane, qnaPageNoPane, commentPageNoPane;
	JButton editBtn, deleteBtn, sendMessageBtn;
...
//	List<ComplComtVO> commentList;
	...
	public Mypage (MemberVO mvoInput){
		userId = mvoInput.getMemId();
		...
		//패널 사이즈, 위치 설정
		mypagePane.setBounds(44, 34, 1394, 794);
		//프레임에 패널 붙이기
		...
		qnaBdTb.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(qnaBdTb.getValueAt(rowIndex, 0) != "") {
					...
					String date = o.toString();
					UserQnaPost userQnaPost = new UserQnaPost(no, title, id, like, viewCnt, date);
					PublicMain.um.changePanel(userQnaPost.getJPanel());
				}
			}
...
//		commentPageNoPane.add(commentNextLb);
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if(btn == sendMessageBtn) {	//메세지 전송 눌렀을 때
			MessageBox mb =new MessageBox();
			PublicMain.um.changePanel(mb.getJPanel());
		}
	}
