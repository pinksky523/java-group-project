...
public class ComplBdMng implements MouseListener{
	ComplBoardDAO cbdao;
	 ...
	 public ComplBdMng() {
		...
		writeBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int re=scn.nextInt();
				if(re==1) {
					if(cbdao.writePostByAdmin(title, content))
...
//		boardTable.setRowSelectionAllowed(false); 
	...
	public void drawPageByState(int pageNo) {
		...
		if(nowPageState==1) {//내용으로검색
			cbVoList=cbdao.getPageUsingContentsByAdmin(pageNo,searchTextField.getText());
		}
			
		if(nowPageState==2) {//아이디로 검색
			cbVoList=cbdao.getPageUsingMemIdByAdmin(pageNo, searchTextField.getText());
		}
		insertPageOnJTable();
	}
